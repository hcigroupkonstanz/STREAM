...
namespace Assets.Modules.Networking
{
    ...
    public class ArtsClient : ObservableModel<ArtsClient>
    {
        const string NAME_PROP = "client_name";

        private static ArtsClient _instance;
        public static ArtsClient Instance
        {
            get
            {
                if (!_instance)
                    _instance = new GameObject($"[ArtsClient]").AddComponent<ArtsClient>();
                return _instance;
            }
        }
        ...
        private void OnEnable()
        {
            if (_instance != null)
            {
                Debug.LogError("Detected multiple ArtsClient instances!");
                Destroy(gameObject);
                return;
            }
